 ...
 */
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Hack for arnold : add param 'fileIndex'
var computeMovieHash = function(torrentUrl, engine, fileIndex){
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    readTorrent(torrentUrl, function(err, torrent) {
        if(err) {
            ...
            function whenEngineReady () {
                // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                // Hack for arnold : select file from param 'fileIndex'
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Hack for arnold : add param 'fileIndex'
exports.computeHash = function(torrentUrl, engine, fileIndex) {
    return computeMovieHash(torrentUrl, engine, fileIndex);
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
