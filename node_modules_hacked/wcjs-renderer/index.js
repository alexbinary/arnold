...
function frameSetup(canvas, width, height, pixelFormat, videoFrame) {
    ...
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    // changes from original module for arnold start here
    ...
    var container_w = canvas.parentElement.clientWidth;
    var container_h = canvas.parentElement.clientHeight;

    var original_frame_w = width;
    var original_frame_h = height;

    var output_w = container_w;
    var output_h = original_frame_h * output_w / original_frame_w;

    if(output_h > container_h) {
      var output_h = container_h;
      var output_w = original_frame_w * output_h / original_frame_h;
    }

    canvas.width = output_w;
    canvas.height = output_h;

    // changes from original module for arnold end here
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
module.exports = {
    init: function(canvas,params,fallbackRenderer) {
        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        // Hack for arnold : use wcjs-prebuilt instead of webchimera.js
